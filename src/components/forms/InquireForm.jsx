...
export default function InquireForm({
  ...
  useEffect(() => {
    if (headingText !== Dopt.heading.text)
      pEle.classList.remove("!border-red-500");
  }, [headingText]);
  ...
  if (prod) {
    return (
      <main className={`${disp ? "block" : "hidden"} font-pRegular z-10`}>
        <aside
          className={`fixed bg-black opacity-50 FullCover`}
          onClick={toggleInq}
        ></aside>
        <section className="absoluteC w-full md:w-[40%] rounded-xl bg-white">
          <div className="flex justify-end md:pt-3 md:pr-3">
            <ImageC
              src="lines/add.svg"
              onclick={toggleInq}
              styles={"h-[3.5rem] w-[3.5rem] hvrPointer"}
            />
          </div>
          <form
            className="FCol w-full gap-5 px-5 md:px-[5rem] pb-[3rem]"
          ...
          >
            ...
            <textarea
              type="text"
              className="InpEnq h-[6rem] resize-none"
              placeholder="Message"
              {...register("message")}
            />
