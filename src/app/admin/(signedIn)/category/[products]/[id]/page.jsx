...
function page({params}) {
  ...
  const { mutate, isLoading, isSuccess, error, data } = useMutation(
    ...
    //   const f = await bkend.get(`/getoneproduct/${store.id}`);
      ...
      const f = await bkend.get(`/getoneproductByName/${params?.id}`);
      ...
      if (productData.discount !== "")
        calcDiscount(productData.discount.percentage, productData.cost);
  ...
  const {
	...
	const f = await bkend.patch(`/updateproduct/${product?._id}`, form, {
		headers: { "Content-Type": "multipart/form-data" },
	  });
    reset();
    mutate();
  ...
  const {
    ...
    const f = await bkend.patch(`/deleteimage/${store.id}`, {
      img: product.imgNames[j],
    });
    mutate();
  });
  ...
  if (product && images) {
    return (
      <>
        <section
          className={`w-full p-3 list-none ${
            deleted ? "opacity-100" : "opacity-100"
          } relative`}
        >
          <form onSubmit={handleSubmit(Submit)}>
            <main className="flex bg-white flex-col w-full px-4 py-3 rounded-lg">
              <div>
                ...
                <li className="mt-3">
                  <label className="text-[rgb(136,135,135)]">Description</label>{" "}
                  <br />
                  <textarea
                    type="text"
                    className="w-full rounded-lg bg-[#f8fafc] border-[1px] border-[rgb(175,173,173)] mt-1 py-2 pl-3 resize-none h-[150px]"
              ...
              <div>
                <aside className="flex justify-between">
                  ...
                  <li>
                    <label className="inline-flex items-center cursor-pointer">
                      <h1 className={`${roboto.className} text-xl mr-4`}>
                        Discount
                      </h1>
                      <input
                        type="checkbox"
                ...
                <li className="mt-3">
                  ...
                  <input
                    type="text"
                    className="w-full rounded-lg bg-[#f8fafc] border-[1px] border-[rgb(175,173,173)] mt-1 py-2 pl-3"
                    {...register("cost", { required: "Cost cannot be Empty" })}
                  ...
                  <aside className="flex justify-between">
                    <div className="w-[45%]">
                      ...
                      <input
                        ...
                        className="w-full rounded-lg bg-[#f8fafc] border-[1px] border-[rgb(175,173,173)] mt-1 py-2 pl-3"
                        {...register("percentage")}
                        onChange={(event) => calcDiscount(event.target.value)}
                        defaultValue={product.discount.percentage}
                      />
                    </div>
                    <div className="w-[45%]">
                      <label className="text-[rgb(136,135,135)]">
              ...
              <div>
                ...
                <li className="mt-3">
                  ...
                  <input
                    type="text"
                    className="w-full rounded-lg bg-[#f8fafc] border-[1px] border-[rgb(175,173,173)] mt-1 py-2 pl-3"
                    {...register("size", { required: "Size cannot be Empty" })}
            ...
            <main
              className={`flex w-full px-4 py-3 bg-white rounded-lg mt-3 justify-between z-10 ${
                deleted ? "opacity-100" : "opacity-100"
              }`}
            >
              <div className="w-[70%]">
                ...
                <li className="mt-3 w-[100%]">
                  ...
                  <br />
                  <div
                    className={`h-[400px] w-[70%] bg-[#f8fafc] mt-2 border-dashed border-[1px] border-[rgb(175,173,173)] p-2 w-full ${
                      deleted ? "opacity-90" : "opacity-90"
                    }`}
                  >
                    <header className="h-[80%] flex justify-evenly flex-wrap content-start overflow-auto pt-1">
                      {images.map((i, j) => (
                        <div
                          className="h-[100px] w-[100px] relative mb-5"
                          key={j}
                        >
                          <Image
                            key={j}
                          ...
                          />
                          <div
                            className={`absolute h-[20px] w-[20px] z-10 -right-1 -top-1 ${
                    ...
                    <aside className={`flex justify-center mt-5`}>
                      <label
                        className={`cursor-pointer bg-[#e3e8fb] border-2 border-[#c5c9fa] text-[#777ef9] px-3 py-2 rounded-lg ${
                      ...
                      <div
                        className="cursor-pointer bg-[#e3e8fb] border-2 border-[#c5c9fa] text-[#777ef9] px-3 py-2 rounded-lg ml-2"
            ...
            <div className="flex justify-center my-5">
              <input
                ...
                className={`px-20 py-4 rounded-lg bg-[#ffd600] ${roboto.className} m-auto`}
              />
            </div>
          </form>
        </section>
